...
├── commands/                13 слэш-команд (промпт-шаблоны)
...
│   ├── job-worker.mjs       отдельный процесс одного запуска Codex
...
└── tests/regressions.mjs    59 регрессионных тестов
```
...
```
MCP-вызов ──► job-worker.mjs ──► codex exec ──► JSONL-журнал задачи
                                                    ▲
...
Воркер переносим (`node job-worker.mjs`), ставит событиям метку времени приёма (сам Codex её не шлёт), атомарно пишет код возврата и оставляет пометку о причине завершения: `timeout`, `cancelled`, `spawn_failed`. `reconcile()` превращает пометку в статус — иначе отмена и таймаут выглядели бы одинаково провалившимся запуском. Дерево процессов убивается через `taskkill /T` на Windows и через группу процессов на POSIX (`proc.mjs`).
