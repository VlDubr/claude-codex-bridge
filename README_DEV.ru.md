...
`codex_ask` больше не падает по таймауту. Он ждёт `wait_seconds` (по умолчанию 90) и, если модель ещё работает, перезапускает тот же запрос фоновой задачей и возвращает собранную ленту. Работа не выбрасывается, и пользователя не просят начать заново.
