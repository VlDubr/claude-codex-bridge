...
2. Вызови `codex_delegate` с этим текстом. Если задача мелкая и однозначная, добавь `model: "gpt-5.4-mini"` — быстрее и дешевле.
3. Опрашивай `codex_status` с нарастающим интервалом. Не дёргай его чаще раза в 30 секунд и не крути опрос вхолостую больше 20 раз.
