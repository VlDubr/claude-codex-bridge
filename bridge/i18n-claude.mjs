...
const MESSAGES = {
  en: {
    ...
    tools_not_allowed: (asked, admin) =>
      ...
      "The administrator configures this set with /tandem:setup --task-tools, and callers cannot expand it.",
    task_write_allowlist_required:
      "Write-enabled claude_task requires a non-empty administrative tool allowlist. Configure it with /tandem:setup --allow-task --task-tools Read,Edit.",
    ...
    proxy_errors: (errors) => `[tandem] tool proxy errors:\n  ${errors.join("\n  ")}\n`,
    proxied_tool_failed: (name, error) => `Proxied tool ${name} failed: ${error}`,
    task_disabled: "claude_task is disabled. Enable it with: /tandem:setup --allow-task",
  ...
  ru: {
    ...
    tools_not_allowed: (asked, admin) =>
      ...
      `[${admin.join(", ")}]. Набор задаётся администратором через /tandem:setup --task-tools и расширению не подлежит.`,
    task_write_allowlist_required:
      "Для claude_task с правом записи нужен непустой административный allowlist. Настрой его: /tandem:setup --allow-task --task-tools Read,Edit.",
    no_tools_after_write_filter: (writeTools, before) =>
    ...
    proxy_errors: (errors) => `[tandem] проброс инструментов:\n  ${errors.join("\n  ")}\n`,
    proxied_tool_failed: (name, error) => `Проброшенный инструмент ${name} завершился ошибкой: ${error}`,
    task_disabled: "claude_task выключен. Включить: /tandem:setup --allow-task",
